...
class ContactController extends Component
{
    ...
    {
        // UpdateTeamUser::dispatch()->onQueue('send-notification');
        // dd($user);
    ...
    public function init()
    {
        ...
        $rolesData = ['mitra', 'member', 'subagent'];

        if ($user->role->role_type == 'mitra') {
            $rolesData = ['subagent', 'member'];
        } else if ($user->role->role_type == 'superadmin') {
            $rolesData = ['mitra', 'member', 'subagent', 'sales', 'leadsales', 'admincs', 'cs', 'warehouse'];
        }
    ...
    public function store()
    {
        ...
        try {
            ...
            if (auth()->user()->role->role_type == 'sales') {
                $role2 = Role::where('role_type', 'mitra')->first();
