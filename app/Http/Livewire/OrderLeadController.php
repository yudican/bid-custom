...
class OrderLeadController extends Component
{
    ...
    public function render()
    {
        ...
        if (in_array($role_type, ['adminsales', 'superadmin', 'leadsales', 'collector'])) {
            $contact_list = User::leftjoin('companies', 'companies.user_id', '=', 'users.id')->leftjoin('role_user', 'role_user.user_id', '=', 'users.id')->leftjoin('roles', 'role_user.role_id', '=', 'roles.id')->select('users.*', 'companies.name as com_name', 'roles.role_type')->whereHas('roles', function ($query) {
                return $query->whereIn('roles.role_type', ['mitra', 'member']);
            })->get();
        } else {
            $contact_list = User::leftjoin('companies', 'companies.user_id', '=', 'users.id')->leftjoin('role_user', 'role_user.user_id', '=', 'users.id')->leftjoin('roles', 'role_user.role_id', '=', 'roles.id')->select('users.*', 'companies.name as com_name', 'roles.role_type')->whereHas('roles', function ($query) {
                $user_id = auth()->user()->id;
                return $query->whereIn('roles.role_type', ['mitra', 'member'])->where('created_by', $user_id)->orWhere('users.id', @$this->contact);
            })->get();
        }

        $contact_list = User::leftjoin('companies', 'companies.user_id', '=', 'users.id')->leftjoin('role_user', 'role_user.user_id', '=', 'users.id')->leftjoin('roles', 'role_user.role_id', '=', 'roles.id')->select('users.*', 'companies.name as com_name', 'roles.role_type')->whereHas('roles', function ($query) {
            return $query->whereIn('roles.role_type', ['mitra', 'member', 'sales', 'adminsales']);
        })->get();
