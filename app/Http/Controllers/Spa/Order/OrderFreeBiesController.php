...
class OrderFreeBiesController extends GpController
{
    ...
    public function listOrderLead(Request $request)
    {
        ...
        if ($role == 'sales') {
            if ($account_id) {
                $orderLead->where('user_created', $user->id)->where('company_id', $account_id);
            } else {
    ...
    public function saveOrderManual(Request $request)
    {
        try {
            ...
            $items = json_decode($request->product_items, true);
    ...
    public function export()
    {
        ...
        Excel::store(new OrderManualExport($orderManual), $file_name, 's3', null, [
            'visibility' => 'public',
        ]);
        return response()->json([
            'status' => 'success',
