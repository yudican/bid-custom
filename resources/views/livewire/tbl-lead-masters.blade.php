                            ...
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">Lead Activity
                                        <div class="pull-right">
                                            @if ((auth()->user()->role->role_type != 'adminsales') && ($lead->status != 1))
                                    ...
                                    <table class="display table table-striped table-hover" id="lead-activity">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Status</th>
                                                <th>Attachment</th>
                                                @if (auth()->user()->role->role_type != 'adminsales')
                                                <th>Action</th>
                                                @endif
                                            </tr>
                                        </thead>
                        ...
                        <div class="tab-pane fade  active show" id="pills-transactive" role="tabpanel" aria-labelledby="pills-transactive-tab">
                            <div class="card">
                                ...
                                <div class="card-body pl-2 pr-2 overflow-x-scroll">
                                    ...
                                    <table width="100%">
                                        <thead class="thead-lightss">
                                            <tr>
                                                ...
                                                @if (auth()->user()->role->role_type != 'adminsales' && auth()->user()->role->role_type != 'superadmin' && auth()->user()->role->role_type != 'leadsales' || $lead->status == '2')
                                        ...
                                        <tbody>
                                            @for ($index = 0; $index < count($inputs); $index++) <tr>
                                                ...
                                                <td class="p-0">
                                                    <div class="d-flex  align-items-center flex-row mt-2">
                                                        <div class="input-group input-spinner mr-3">
                                                            ...
                                                            <button class="btn btn-light btn-xs" type="button" wire:click="getPrice({{$product_id[$index]}},{{$index}},'min')" {{(in_array(auth()->user()->role->role_type, ['adminsales', 'superadmin', 'leadsales']) || $lead->status == '2'
                                                                )?'disabled':''}}> <i class="fas fa-minus"></i> </button>

                                                            <button class="btn btn-light btn-xs" type="button"> {{$qty[$index]}} </button>

                                                            <button class="btn btn-light btn-xs" type="button" wire:click="getPrice({{$product_id[$index]}},{{$index}},'plus')" {{(in_array(auth()->user()->role->role_type, ['adminsales', 'superadmin', 'leadsales']) || $lead->status == '2'
                                                ...
                                                <td class="p-0">
                                                    <input id="price.{{$index}}" value="" name="price.{{$index}}" wire:model="price.{{$index}}" type="text" class="form-control" {{(auth()->user()->role->role_type == 'adminsales' || auth()->user()->role->role_type == 'superadmin' || $lead->status ==
                                                    '2')?'readonly':''}}>
                                                </td>
                                                @if ((auth()->user()->role->role_type != 'adminsales' && auth()->user()->role->role_type != 'superadmin' && auth()->user()->role->role_type != 'leadsales') && ($lead->status != 1 && $lead->status != 2) && count($activity) > 0 )
                                                <td class="p-0">
                                                    @if ($index > 0)
                                    ...
                                    @endif
                                    <br><br><br>
                                    @if (!in_array(auth()->user()->role->role_type, ['adminsales', 'leadsales']) && count($productneeds) > 0 && count($activity) > 0 && !in_array($lead->status, ['1', '2', '6']))
                                    ...
                                    @endif
                                    @if (!in_array(auth()->user()->role->role_type, ['adminsales', 'leadsales']) && count($productneeds) > 0 && count($activity) > 0 && $lead->status == 1 && $lead->status != 2)
                                    ...
                                    @endif
                                    @if (!in_array(auth()->user()->role->role_type, ['adminsales', 'leadsales']) && $lead->status == 6)
