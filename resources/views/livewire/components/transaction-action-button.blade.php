<div class="list-group-item-figure" id="{{$trans->id}}" wire:key="item-{{ $trans->id }}">
  <div class="dropdown">
    ...
    <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; transform: translate3d(-124px, 25px, 0px); top: 0px; left: 0px; will-change: transform;">
      @if (isset($role) && $trans)
      @if (in_array($role->role_type, ['admin', 'superadmin', 'finance', 'warehouse', 'adminsales']))
      ...
      @if ($role->role_type == 'finance' && $trans->status < 3) @if ($trans->confirmPayment)
        ...
        @if(in_array($role->role_type, ['warehouse','adminsales','superadmin']) && $trans->status_delivery == 1 && $trans->status == '7')
        @if (in_array($role->role_type, ['adminsales','superadmin']) && $trans->label)
        <a href="{{$trans->label->label_url}}" target="_blank" class="dropdown-item">Cetak Label</a>
        @endif
        @if (in_array($role->role_type, ['warehouse']))
        <a href="#" wire:click="packingProcess({{$trans->id}})" class="dropdown-item">Proses Pengemasan</a>
        @endif

        @endif

        @if(in_array($role->role_type, ['warehouse','adminsales','superadmin']) && $trans->status_delivery == 21)
        @if (in_array($role->role_type, ['adminsales','superadmin']) && $trans->label)
        <a href="{{$trans->label->label_url}}" target="_blank" class="dropdown-item">Cetak Label</a>
        @endif
        ...
        @if($role->role_type == 'warehouse' && $trans->status_delivery == 3)
        ...
        @if (in_array($role->role_type, ['admin', 'superadmin', 'finance', 'adminsales']) && $trans->status == 3)
        <a href="#" wire:click="assignWarehouse({{$trans->id}})" class="dropdown-item">Assign To Warehouse</a>
        @endif
        @elseif (in_array($role->role_type, ['mitra','subagent']))
