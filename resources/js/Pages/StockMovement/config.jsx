...
const purchaseOrderListColumn = [
  ...
  },
  {
    title: "End Stock",
    render: (text, record) => {
        var incoming = parseInt((!record.purchase_delivered)?0:record.purchase_delivered) + parseInt((!record.product_return)?0:record.product_return) + parseInt((!record.sales_return)?0:record.sales_return)
        var outcoming = parseInt((!record.stock)?0:record.stock) + parseInt((!record.return_suplier)?0:record.return_suplier) + parseInt((!record.sales)?0:record.sales) + parseInt((!record.transfer_out)?0:record.transfer_out)
        return parseInt((!record.begin_stock)?0:record.begin_stock) + incoming - outcoming;
    },
  },
  {
    title: "End Forecast",
    render: (text, record) => {
        return parseInt((!record.stock)?0:record.stock) + parseInt((!record.sales)?0:record.sales);
    },
  },
//   {
//     title: "Status",
