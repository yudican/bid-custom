...
const OrderFreebiesDetail = () => {
  ...
  const loadDetailOrderLead = () => {
    setLoading(true)
    axios.get(`/api/freebies/${params.uid_lead}`).then((res) => {
  ...
  const assignWarehouse = () => {
    axios
      .get(`/api/freebies/assign-warehouse/${params.uid_lead}`)
      .then((res) => {
        loadDetailOrderLead()
  ...
  const handleChangeCell = ({ value, dataIndex, key, reminder_id }) => {
    ...
    axios
      .post(`/api/freebies/reminder/update`, {
        field: dataIndex,
        value,
        reminder_id,
        uid_lead: params.uid_lead,
  ...
  const handleAddReminder = () => {
    axios
      .post(`/api/freebies/reminder/save`, {
        uid_lead: params.uid_lead,
      })
      .then((res) => {
        loadDetailOrderLead()
  ...
  const handleClickCell = ({ key, type }) => {
    if (type === "plus") {
      ...
      axios
        .get(`/api/freebies/reminder/delete/${key}`)
        .then((res) => {
          loadDetailOrderLead()
        })
        .catch((err) => {})
  ...
  const handleChangeKurir = (courier) => {
    if (orderDetail?.courier !== courier) {
      axios
        .post(`/api/freebies/change-courier`, {
          courier,
  ...
  const setClosed = () => {
    axios.get(`/api/freebies/closed/${orderDetail.uid_lead}`).then((res) => {
      loadDetailOrderLead()
  ...
  const handleVerifyBilling = (value, status) => {
    ...
    axios
      .post(`/api/freebies/billing/verify`, { status, ...value })
      .then((res) => {
        loadDetailOrderLead()
  ...
  const onFinishSaveResi = (values) => {
    axios
      .post("/api/freebies/shipping/save", {
        ...values,
        uid_lead: params.uid_lead,
      })
      .then((res) => {
        ...
        <RenderIf isTrue={item.label === "Kode Unik"}>
          <Table.Summary.Cell index={6}>
            <UpdateUniqueCode
              ...
              url={"/api/freebies/update/kode-unik"}
              disabled={disabled}
            />
          </Table.Summary.Cell>
        </RenderIf>
        <RenderIf isTrue={item.label === "Ongkir"}>
          <Table.Summary.Cell index={6}>
            <ModalOngkosKirim
              ...
              refetch={loadDetailOrderLead}
              url={`/api/freebies/update/ongkir/${orderDetail.uid_lead}`}
            />
          </Table.Summary.Cell>
        </RenderIf>
