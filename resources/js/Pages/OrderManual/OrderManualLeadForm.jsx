import { Button, Card, Divider, Form, Input, Select, Switch, Table } from "antd"
...
const OrderManualLeadForm = () => {
  ...
  const updateSalesOrder = (address_id) => {
    axios
      .post("/api/order-manual/form/update", {
        uid_lead,
        contact: seletedContact.value,
        address_user_id: address_id,
        type: "manual",
      })
      .then((res) => {
        toast.success("Berhasil Pilih Alamat", {
          position: toast.POSITION.TOP_RIGHT,
        })
        loadUserAddress(seletedContact.value)
        loadProductDetail(true)
      })
      .catch((err) => {
        const { message } = err.response.data
        toast.error("Gagal Pilih Alamat", {
          position: toast.POSITION.TOP_RIGHT,
        })
      })
  }
