...
const MappingWarehouseList = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        const newData = data.map((item) => {
          return {
            id: item.id,
            warehouse_id: item.tiktok_warehouse_id,
            warehouse_name: item.warehouse_name,
            warehouse_city: item.warehouse_city,
            warehouse_contact: item?.warehouse_contact || "-",
            warehouse_phone: item?.warehouse_phone || "-",
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadContact(`/api/mapping/warehouse`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadContact(`/api/mapping/warehouse`, 10, data)
  }
  ...
  const handleSyncron = () => {
    setLoadingExport(true)
    axios
      .get(`/api/mapping/warehouse/syncron/`)
      .then((res) => {
        const { data } = res.data
        // window.open(data)
        setLoadingExport(false)
  ...
  if (loadingExport) {
    return (
      <Layout title="List Mapping Warehouse">
        <LoadingFallback />
      </Layout>
    )
  }
  ...
  return (
    <Layout rightContent={rightContent} title="List Mapping Warehouse">
